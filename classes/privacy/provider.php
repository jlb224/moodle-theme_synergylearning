...
/**
 * Privacy Subsystem implementation for theme_synergylearning.
 *
...
namespace theme_synergylearning\privacy;
...
/**
 * The synergylearning theme stores a user preference data.
 *
 * @copyright  2018 Andrew Nicols <andrew@nicols.co.uk>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class provider implements
    ...
    public static function export_user_preferences(int $userid) {
        ...
        if (isset($draweropenindexpref)) {
            $preferencestring = get_string('privacy:drawerindexclosed', 'theme_synergylearning');
            if ($draweropenindexpref == 1) {
                $preferencestring = get_string('privacy:drawerindexopen', 'theme_synergylearning');
            }
            \core_privacy\local\request\writer::export_user_preference(
                'theme_synergylearning',
        ...
        if (isset($draweropenblockpref)) {
            $preferencestring = get_string('privacy:drawerblockclosed', 'theme_synergylearning');
            if ($draweropenblockpref == 1) {
                $preferencestring = get_string('privacy:drawerblockopen', 'theme_synergylearning');
            }
            \core_privacy\local\request\writer::export_user_preference(
                'theme_synergylearning',
